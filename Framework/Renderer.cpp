...
void Renderer::Initialize(int _xResolution, int _yResolution)
{
	...
	M_ASSERT(m_renderer != nullptr, "Failed to initialze SDL renderer.");
...
SDL_Texture* Renderer::GetSDLTexture(Texture* _texture)
{
	...
	m_surface = SDL_CreateRGBSurfaceFrom(asset->GetData() + _texture->GetImageInfo()->DataOffset, ii->Width, ii->Height, ii->BitsPerPixel,
		ii->Width * ii->BitsPerPixel / 8, 0x00FF0000, 0x0000FF00,0x000000FF, 0xFF000000 ); //Texure Masks. We divide by 8 to convert bits to bytes
...
void Renderer::RenderTexure(Texture* _texture, Point _point)
{
